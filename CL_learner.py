...
class Seq2SeqToD(pl.LightningModule):
    ...
    def __init__(self,args):
        ...
        if "t5" in args.model_checkpoint:
            ...
            if(args.CL == "ADAPTER"):
                ...
                # model.add_adapters(bottleneck_size=args.bottleneck_size, adapter_num=args.number_of_base_adpt)
            else:
        ...
        self.task_id2center = {}
        # self.evaluator = GPT2Model.from_pretrained('gpt2')

    def compute_Distance(self, batch, task_id=-1, device="cuda"):
        with torch.no_grad():
            output = []
            batch_hidden = self.getLastHiddenStateGPT(input_ids=batch["input_id_PPL"].to(device),
                                                      attention_mask=None,
                                                      labels=None,
                                                      task_id=task_id).cuda()
            for i in range(len(batch_hidden)):  # each_sentence is hidden
                center = self.task_id2center[task_id]
                sentence_vector = torch.mean(batch_hidden[i], axis=0)
                distance = self.EuclideanDistance(sentence_vector, center.cuda())
                output.append((distance.item(), task_id))
            return output

    def EuclideanDistance(self, t1, t2):
        dist = torch.dist(t1, t2, p=2)  # L2
        return dist

    def getLastHiddenState(self, input_ids, attention_mask, labels, task_id):
        with torch.no_grad():
            _ = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=labels, task_id=task_id)
            batch_hidden = self.model.last_hidden_states
        return batch_hidden

    def getLastHiddenStateGPT(self, input_ids, attention_mask, labels, task_id):
        with torch.no_grad():
            batch_hidden = GPT2Model.from_pretrained('gpt2')(input_ids=input_ids, attention_mask=attention_mask,
    ...
    def compute_PPL(self,batch,task_id=-1,device='cuda'):
        with torch.no_grad():
            lm_logits, *_ = self.model(
                            ...
                            labels=None,
                            task_id=task_id,
                            task_string=batch["task_id"][0][2:-2]
                            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if(self.CL == "ADAPTER"):
            ...
            # print("evaluate here task_id:", task_id)

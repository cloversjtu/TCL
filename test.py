...
def test_generation_GPT2BATCH(model, tokenizer, input_text, device, do_sample=False, temperature=1.0,top_k=0,top_p=0,max_length=30, task_id=-1, task_string=None):
    ...
    for step in range(max_length):
        ...
        if task_id == -1:
            ...
            # print("task_string in test_generation_GPT2BATCH:", task_string)
            outputs = model(input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past, task_id=task_id, task_string=task_string)
...
def test_model_seq2seq_ADAPTER(args,model,tokenizer,test_loader,test_dataset,time="0_['']",max_seen_task=0):
    ...
    for pred_task_id, task_loader in tqdm(test_dataset_by_predicted_id.items(),total=len(test_dataset_by_predicted_id)):
        ...
        for idx_b, batch in tqdm(enumerate(task_loader),total=len(task_loader)):
            with torch.no_grad():
                # print("test:pred_task_id:{},task_string:{},split task_string:{}", pred_task_id, model.task_list_seen[pred_task_id], model.task_list_seen[pred_task_id][2:-2])
                ...
                value_batch,_ = test_generation_GPT2BATCH(model=model.model,
                                                    ...
                                                    task_string= model.task_list_seen[pred_task_id][2:-2])

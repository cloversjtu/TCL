...
def evaluate_API(pred,gold, hist, k):
    intent_accuracy = []
    turn_level_slot_acc = []
    turn_level_joint_acc = []
    subset_acc = []
    for p, g, h in zip(pred,gold, hist):
        API_G = {}
        API_P = {}
        p = p+" "
        if(g!=""):
            ...
            if(p!="" and "(" in p and ")"): ## means the predicted text is an API
                ...
                if set(state_G.items()).issubset(set(state_P.items())) and len(set(state_G.items())) < len(set(state_P.items())):
                    # print("===============================================================")
                    print("key:", k)
                    print("hist:", h)
                    ...
                    # if k=="['sgd_buses']":
                    #     break
                # if set(state_G.items()).issubset(set(state_P.items())):
                    subset_acc.append(1)
                ...
                subset_acc.append(0)
    print("len of subset:", sum(subset_acc))
    print("len of turn_level_joint_acc:", sum(turn_level_joint_acc), len(turn_level_joint_acc))
    print("*****************************************")
    return {"intent_accuracy":np.mean(intent_accuracy),
            "turn_level_joint_acc":np.mean(turn_level_joint_acc),
            "subset_acc": np.mean(subset_acc)}
...
def evaluate(args,path,names,ent={}):
    ...
    for r in results_json:
        if(r['spk']=='SYSTEM'):
            ...
            domain_API[r['task_id']]["hist"].append(r['hist'])
    ...
    for k,v in domain_API.items():
        ...
        if args.task_type =="E2E" or args.task_type =="DST":
            T_API[k] = evaluate_API(v["pred"],v["gold"], v["hist"], k)
...
def score_folder():
    ...
    for folder in folders:
        ...
        if(args.task_type == "INTENT"):
            ...
            INTENT = np.mean([v["intent_accuracy"] for k,v in res["API"].items()])
            JGA = np.mean([v["turn_level_joint_acc"] for k,v in res["API"].items()])
            SAC = np.mean([v["subset_acc"] for k,v in res["API"].items()])
            BLEU = np.mean([v for k,v in res["BLEU"].items()])
            EER = np.mean([v for k,v in res["EER"].items()])
            RESULT.append({"Name":folder.split("/")[-1].split("_")[0],"INTENT":INTENT,"JGA":JGA,"BLEU":BLEU,"EER":EER, "SAC":SAC})
